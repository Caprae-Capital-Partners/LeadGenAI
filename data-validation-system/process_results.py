...
def generate_confidence_chart(df, output_dir):
    ...
    if not confidence_cols:
        print("Warning: No confidence columns found in the data")
        # Create a simple dummy chart
    ...
    if not data:
        print("Warning: No valid confidence data to plot")
        plt.text(0.5, 0.5, "No valid confidence data to plot", 
                 horizontalalignment='center', verticalalignment='center')
        chart_path = os.path.join(output_dir, 'confidence_distribution.png')
        plt.savefig(chart_path)
        return chart_path
...
def generate_html_report(df, summary_path, charts_dir):
    ...
    if summary_path and os.path.exists(summary_path):
        try:
            ...
            print(f"Warning: Could not parse summary file {summary_path}. File may be empty or contain invalid JSON.")
    ...
    if 'Business Name' in df.columns:
        display_columns.append('Business Name')
            ...
            if col in df.columns:
                ...
                if pd.isna(value):  # Handle NaN values
                    ...
                    try:
                        ...
                        conf_class = 'confidence-high' if value_float >= 0.8 else 'confidence-medium' if value_float >= 0.6 else 'confidence-low'
                        html += f'<td><span class="{conf_class}">{value_float:.2f}</span></td>'
...
def main():
    ...
    try:
        ...
        try:
            ...
            print(f"Error: Could not parse {args.input}. Trying with more flexible parsing...")
            df = pd.read_csv(args.input, engine='python', on_bad_lines='skip')
            print(f"Loaded validation results with alternative parser ({len(df)} records)")
        except Exception as e:
