            ...
            if event['type'] == 'checkout.session.completed':
                ...
                if 'payment_intent' in session:
                    try:
                        ...
                        current_app.logger.warning(f"[Webhook] Could not retrieve payment method info: {str(e)}")
        ...
        try:
            ...
            try:
                ...
                if not customers.data:
                    # If user is on a paid tier but no Stripe customer found, reset to free
                ...
                if cancellation_type == 'immediate':
                    ...
                    current_app.logger.info(f"Immediately canceled subscription {subscription.id} for user {user.user_id}")
    ...
    def _handle_local_cancellation(user, user_sub):
        ...
        try:
            ...
            if free_plan:
                ...
                user_sub.credits_remaining = free_plan.initial_credits if free_plan.initial_credits is not None else 10
