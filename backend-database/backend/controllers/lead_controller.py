...
class LeadController:
    ...
    def search_leads_by_industry_location(industry, location, current_user=None):
        ...
        for lead in leads:
            lead_dict = {}
            # Check user tier
            print(f"[DEBUG] current_user.tier exists: {hasattr(current_user, 'tier')}, tier: {getattr(current_user, 'tier', None)}, role: {getattr(current_user, 'role', None)}, username: {getattr(current_user, 'username', None)}")
            print(f"[DEBUG] Is Free: {getattr(current_user, 'tier', None) == 'free'}, Is User: {getattr(current_user, 'tier', None) == 'user'}")
